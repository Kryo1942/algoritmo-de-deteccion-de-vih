(function () {
  ...
  function landingCard(vaccine) {
    return "<a class='vax-card' href='" + vaccineUrl(vaccine.slug) + "'><div><div class='vax-chip-row'><span class='vax-chip " + vaccine.tone + "'>" + esc(vaccine.short) + "</span><span class='vax-chip'>Vacuna</span></div><h3>" + esc(vaccine.title) + "</h3><p>" + esc(vaccine.protects) + "</p></div><span class='vax-card-link'>Ver vacuna</span></a>";
  }

  function credits() {
    return "<section class='vax-credits'><div><h2>Equipo del protocolo de VIH</h2><ol>" + teamNames().map(function (name) { return "<li>" + esc(name) + "</li>"; }).join("") + "</ol></div><div><h2>Revisión académica</h2><p><strong>Revisado por el Dr. Noé Macías Segura</strong></p><p>Departamento de Inmunología, UANL.</p></div></section>";
  }

  function sources(items) {
    return "<section class='vax-panel'><div class='vax-section-head'><div><span class='vax-eyebrow'>Referencias</span><h2>Fuentes consultadas</h2></div></div><div class='vax-sources'>" + items.map(function (source) { return "<a class='vax-source' target='_blank' rel='noopener noreferrer' href='" + source[1] + "'><span>" + esc(source[0]) + "</span>" + esc(source[1].replace(/^https?:\/\//, "")) + "</a>"; }).join("") + "</div></section>";
  }

  function renderLanding() {
    app.innerHTML = topbar("vaccination") + "<main class='vax-shell'><section class='vax-hero'><div><span class='vax-eyebrow'>UANL · Departamento de Inmunología</span><h1>Vacunación clínica</h1><p>Utilidad, esquema general, prevalencia, recurrencia, protección y efectos adversos de vacunas frecuentes.</p><div class='vax-toolbar'><a class='vax-button primary' href='#vacunas'>Ver vacunas</a><button class='vax-button' type='button' onclick='window.print()'>Imprimir</button></div></div><aside class='vax-hero-aside'><strong>Protección comunitaria</strong><ul class='vax-list'><li>Preparan defensas antes de la exposición.</li><li>Reducen complicaciones y hospitalizaciones.</li><li>Requieren esquema completo o refuerzos según edad y riesgo.</li></ul></aside></section><div class='vax-section-head' id='vacunas'><div><span class='vax-eyebrow'>Vacunas</span><h2>Vacunas incluidas</h2></div></div><section class='vax-grid'>" + vaccines.map(landingCard).join("") + "</section>" + credits() + "</main>";
    setupVaxIndexMenu();
  }
  ...
  function renderDetail(vaccine) {
    ...
    app.innerHTML = topbar("vaccination") + "<main class='vax-shell'><a class='vax-back' href='" + vaccinationUrl() + "'>Volver a vacunación</a><section class='vax-detail-hero'><div><span class='vax-eyebrow'>Vacuna</span><h1>" + esc(vaccine.title) + "</h1><p>" + esc(vaccine.protects) + "</p><div class='vax-chip-row'><span class='vax-chip " + vaccine.tone + "'>" + esc(vaccine.short) + "</span><span class='vax-chip'>Prevención</span></div></div><aside class='vax-detail-aside'><strong>Protección</strong><p>Las vacunas preparan defensas antes de la exposición. La mayoría de molestias posteriores son leves y breves.</p></aside></section><section class='vax-panel'><div class='vax-section-head'><div><span class='vax-eyebrow'>Datos clínicos</span><h2>Datos principales</h2></div></div><div class='vax-infographic'>" + tile("primary", "Para qué sirve", vaccine.use) + tile("blue", "Esquema general", vaccine.schedule) + tile("green", "Prevalencia", vaccine.prevalence) + tile("gold", "Recurrencia", vaccine.recurrence) + tile("red", "Qué previene", vaccine.protects) + "</div><div class='vax-toolbar'><button class='vax-button primary' type='button' data-adverse>Ver efectos adversos</button><button class='vax-button' type='button' onclick='window.print()'>Imprimir</button></div><section class='vax-panel vax-adverse' id='adversePanel' hidden><span class='vax-eyebrow'>Seguridad</span><h2>Efectos adversos</h2><p>La mayoría son leves, duran poco tiempo y desaparecen solos. No significan que la vacuna haya causado la enfermedad.</p><ul>" + adverseList + "</ul><div class='vax-note'><strong>Cuándo consultar:</strong> " + esc(vaccine.warning) + "</div></section></section>" + sources(vaccine.sources) + credits() + "</main>";
    setupVaxIndexMenu();
